...
def save_data(phi,U):
    ...
    return np.vstack(( np.reshape( (phi[1:-1,1:-1].T) ,(nv,1),order='F')[ins] , \
                      np.reshape( (c_tilde[1:-1,1:-1].T) ,(nv,1),order='F')[ins] ) )
...
Uf = U[1:-1,1:-1].T
xxs=np.reshape(np.reshape(xx,(nv))[ins],(nzs,nxs))
zzs=np.reshape(np.reshape(zz[1:-1,1:-1].T,(nv))[ins],(nzs,nxs))
save(os.path.join(direc,filename),{'xx':xxs*W0,'zz':zzs*W0,'y':Tishot,'dt':dt*tau0,'nx':nxs,'nz':nzs,'t':t*tau0,'mach_time':end-start})
