...
p = PARA.phys_parameter(sys.argv[2],sys.argv[3])
s = PARA.simu_parameter(p.rb)
...
traj_filename = 'macro_output_p.mat'
G_cen = G_traj[:,int((num_sam-1)/2)]
R_cen = R_traj[:,int((num_sam-1)/2)]
y_cen = y_traj[:,int((num_sam-1)/2)]
dend = np.argmin(np.absolute(y_cen+5e-4))

save(os.path.join(s.direc, traj_filename), {'G_cen':G_cen,'R_cen':R_cen,'x_traj':x_traj,'y_traj':y_traj,'T_traj':T_traj,'G_traj':G_traj,'R_traj':R_traj,'beta_traj':beta_traj, 'time_traj':time_traj, 't_start':istart*dt })
print('Gmax: ',G_cen[0])
print('the end y coordinate',y_cen[dend], 'Rmax: ',R_cen[dend])
end =time.time()
