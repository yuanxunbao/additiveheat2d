...
def BC_421(ph, ph_BC): # size of ph is nx+2*ny+2, ph_BC is 2(nx+ny), ha_wd
    ...
    if (  i< ny and j < ha_wd ):
        ph_BC[i,j] = ph[j+ha_wd,i+1]
        ph_BC[i+ny,j] = ph[-1-j-ha_wd,i+1]
    if (  i < nx and j < ha_wd ):
        ph_BC[i+2*ny,j] = ph[i+1,j+ha_wd]
        ph_BC[i+2*ny+nx,j] = ph[i+1,-1-j-ha_wd]
...
BCrecv = cuda.device_array([2*nx+2*ny,ha_wd],dtype=np.float64)
'''
-------------------------------------------------------------------------------------------------
GPU CALCULATIONS
-------------------------------------------------------------------------------------------------
