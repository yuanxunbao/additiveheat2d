...
@cuda.jit
def setNBC_gpu(ps,ph,U,dpsi, px, py, nprocx, nprocy):
...
def BC_421(ps, ph, U, dpsi, BC): # size of ph is nx+2*ny+2, ph_BC is 2(nx+ny), ha_wd
    ...
    if (  i< ny and j < ha_wd ):
        ...
        BC[i+ny,0] = ps[-2-j,i+1]
        BC[i+ny,1] = ph[-2-j,i+1]
        BC[i+ny,2] = U[-2-j,i+1]
        BC[i+ny,3] = dpsi[-2-j,i+1]

    if (  i < nx and j < ha_wd ):
        BC[i+2*ny,0] = ps[i+1,j+1]
        BC[i+2*ny,1] = ph[i+1,j+1]
        BC[i+2*ny,2] = U[i+1,j+1]
        BC[i+2*ny,3] = dpsi[i+1,j+1]
      
        BC[i+2*ny+nx,0] = ps[i+1,-2-j]
        BC[i+2*ny+nx,1] = ph[i+1,-2-j]
        BC[i+2*ny+nx,2] = U[i+1,-2-j]
        BC[i+2*ny+nx,3] = dpsi[i+1,-2-j]
...
def BC_124(ps, ph, U, dpsi, BC): # size of ph is nx*ny, nbh_BC is 2(nx+ny), ha_wd
    ...
    if (  i< ny and j < ha_wd ):
        ...
        ps[-1-j,i+1]   = BC[i+ny,0]
        ph[-1-j,i+1]   = BC[i+ny,1]
        U[-1-j,i+1]    = BC[i+ny,2]
        dpsi[-1-j,i+1] = BC[i+ny,3]


    if (  i < nx and j < ha_wd ):

        ps[i+1,j]   = BC[i+2*ny,0]
        ph[i+1,j]   = BC[i+2*ny,1]
        U[i+1,j]    = BC[i+2*ny,2]
        dpsi[i+1,j] = BC[i+2*ny,3]

        ps[i+1,-1-j] = BC[i+2*ny+nx,0]
        ph[i+1,-1-j] = BC[i+2*ny+nx,1]
        U[i+1,-1-j] = BC[i+2*ny+nx,2]
        dpsi[i+1,-1-j] = BC[i+2*ny+nx,3]

   # the order of recv is [0,0], [-1,0], [0,-1], [-1,-1] 
     
    ps[0,0]= BC[-4,0];   ph[0,0]= BC[-4,1];  U[0,0]= BC[-4,2];  dpsi[0,0] =BC[-4,3];
    ps[-1,0]=BC[-3,0];   ph[-1,0]=BC[-3,1];  U[-1,0]=BC[-3,2];  dpsi[-1,0]=BC[-3,3];
    ps[0,-1]=BC[-2,0];   ph[0,-1]=BC[-2,1];  U[0,-1]=BC[-2,2];  dpsi[0,-1]=BC[-2,3];
    ps[-1,-1]=BC[-1,0];  ph[-1,-1]=BC[-1,1]; U[-1,-1]=BC[-1,2]; dpsi[-1,-1]=BC[-1,3];
...
def BC_comm(sendbuf, recvbuf, nx ,ny):
    ...
    if ( px < nprocx-1 and py < nprocy-1):
           # right_send = phi_gpu[-2*ha_wd:-ha_wd,:].reshape((ha_wd,ny+2*ha_wd))
            comm.Send([sendbuf[-1,:],MPI.DOUBLE], dest = rank+1+nprocx, tag=5)
...
filename = filename + 'corn' +'rank'+str(rank)
...
if px == 0:
    if py == 0:
        nx = nx+1; nz=nz+1;
        x = np.linspace(lminx,lminx+len_blockx,num=nx)
        z = np.linspace(lminz,lminz+len_blockz,num=nz)
    else:
        nx = nx+1; nz=nz;
        x = np.linspace(lminx,lminx+len_blockx,num=nx)
        z = np.linspace(lminz+dz,lminz+len_blockz,num=nz)

elif px > 0 and py == 0:
    nx = nx; nz=nz+1;
    x = np.linspace(lminx+dx,lminx+len_blockx,num=nx)
    z = np.linspace(lminz,lminz+len_blockz,num=nz)
...
ha_wd = 1
nv= (nz+2)*(nx+2) #number of variables
...
DPSI = np.zeros((nv,nts+1), dtype=np.float64)
'''
-------------------------------------------------------------------------------------------------
INIT. TIP POSITION
-------------------------------------------------------------------------------------------------
...
#    c_tilde = ( 1+ (1-k)*U )*( k*(1+phi)/2 + (1-phi)/2 )
    ...
    return np.reshape(psi,     (nv,1), order='F') , \
           np.reshape(phi, (nv,1), order='F') , \
           np.reshape(U, (nv,1), order='F') , \
           np.reshape(dpsi, (nv,1), order='F')
...
dpsi = np.zeros((nx+2,nz+2))
# save initial data
#order_param[:,[0]], conc[:,[0]], zz_mv[:,0] = save_data(phi_cpu, U_cpu, z_cpu )
op_psi[:,[0]], order_param[:,[0]], conc[:,[0]], DPSI[:,[0]] = save_data(psi_cpu, phi_cpu, U_cpu, dpsi )
t_snapshot[0] = 0.
...
BCsend = cuda.device_array([2*nx+2*nz,4],dtype=np.float64);
BCrecv = cuda.device_array([2*nx+2*nz+4,4],dtype=np.float64)
...
setNBC_gpu[bpg,tpb](psi_old,phi_old,U_new,dPSI, px, py, nprocx, nprocy)
...
for kt in range(int(Mt/2)):
   ...
   # setBC_gpu[bpg,tpb](psi_new, phi_new, U_old, dPSI)
    ...
    setNBC_gpu[bpg,tpb](psi_new,phi_new,U_old,dPSI, px, py, nprocx, nprocy)
...
#        cqois[:,kqs] = conc_var(phi_cp,cnc_cp)
    ...
    if (2*kt+2)%kts==0:
      ...
      # order_param[:,[kk]], conc[:,[kk]], zz_mv[:,kk] = save_data(phi,U,z_cpu)
       ...
       op_psi[:,[kk]], order_param[:,[kk]], conc[:,[kk]], DPSI[:,[kk]] = save_data(psi, phi, U, dpsi )
...
save(os.path.join(direc,filename+'.mat'),{'psi':op_psi,'dpsi':DPSI,'order_param':order_param, 'conc':conc, 'xx':xx*W0, 'zz_mv':zz_mv*W0-ztip_qoi[0],'dt':dt*tau0,\
