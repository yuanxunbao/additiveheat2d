...
def spacings(phi, Ntip, lxd, dxd, mph):
    ...
    if mph == 'cell':
        ...
        sec_spac = np.mean( sec_spac_arr[ns:] )
...
def secondary_spacing(cells, sigmai, phi_cp, dxd):
    ...
    for jj in range(1, ncells-1):
        
        coors[2*jj-1] = mid[jj]
        coors[2*jj] = ncells[jj]
...
def tilted_angle(ph_up, ph_down, Nd, Npri, dire): # Npri is the number of grids of primary spacing
    ...
    ## now this is noly work for alpha>0
    shift = 0
    ...
    for  ii in range(Npri):
        ...
        ph_up = np.hstack((ph_up[-1],ph_up[:-1]))
        diff_temp = norm(ph_up-ph_down)
