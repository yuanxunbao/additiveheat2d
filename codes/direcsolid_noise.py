...
from math import pi
import numpy as np
from ds_input_alcu import phy_parameter, simu_parameter
...
def noisy_initial(): # properties/parameters of interests
  
    pl_in = int((nz-1)/20)-1
    z0 = pl_in*dz
    nmag = 0.2
    #using a flat(uniform) distibution
    rand_gene  = np.random.rand(1,nx)
    noise = (rand_gene-0.5)*nmag
    
    
    phi0 = -np.tanh( ( zz-z0 ))
    phi0[[pl_in],:] = phi0[[pl_in],:] + noise
