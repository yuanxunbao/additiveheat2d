...
def move_frame(Ntip, psi, phi, U, zz, dPSI, dPSIb, psib, Ub, flag):
    ...
    if Ntip==Nset:
        ...
        dPSIb = dPSI[:,1]; psib = psi[:,1]; Ub = U[:,1]
   # else: 
        #flag =0
    
    return  psi, U, zz, dPSIb, psib, Ub, Ntip, zz[3,Ntip], flag
...
def set_BC(u,BCx,BCy,vecb):
    ...
    if BCy == 2 :   # moving-frame set the bottom BC already, top is no flux
...
dPSIb = dPSI[:,0]; psib = psi[:,0]; Ub = U[:,0]; flag = 1;
...
save(os.path.join(direc,filename),{'xx':xx*W0,'zz':zz[1:-1,1:-1].T*W0,'y':Tishot,'Ntip':Ntip_arr,'ztip':ztip_arr,'dt':dt*tau0,'nx':nx,'nz':nz,'t':t*tau0,'mach_time':end-start})
