...
def atheta(ux, uz):
    ...
    if (MAG_sq2 > eps**2):
        ...
        return a_s*( s1 + epsilon*(ux2**2 + uz2**2) / MAG_sq2   )
        # return uz/MAG_sq2
    else:
        return a_s
...
def _rhs_psi(ps,ph,U,zz):
    ...
    rhs_psi = ((JR-JL) + (JT-JB) + extra) * hi**2 + \
               sqrt2*ph[0,0] - lamd*(s1-ph[0,0]**2)*sqrt2*(U[0,0] + Up) 
...
def _rhs_U(U,ph,psi_t):
    ...
    phz = phipjp - phipjm
    phn2 = phx**2 + phz**2
    nx = phx / np.sqrt(phn2) if (phn2 > eps) else s0
    
    jat    = p5*(s1+(s1-k)*U[0,0])*(s1-ph[0,0]**2)*psi_t[0,0]
    jat_ip = p5*(s1+(s1-k)*U[1,0])*(s1-ph[1,0]**2)*psi_t[1,0]
    ...
    jat_im = p5*(s1+(s1-k)*U[-1,0])*(s1-ph[-1,0]**2)*psi_t[-1,0]
    ...
    phz = ph[0,1]-ph[0,0]
    phn2 = phx**2 + phz**2
    nz = phz / np.sqrt(phn2) if (phn2 > eps) else s0
          
    jat_jp = p5*(s1+(s1-k)*U[0,1])*(s1-ph[0,1]**2)*psi_t[0,1]      
    ...
    jat_jm = p5*(s1+(s1-k)*U[0,-1])*(s1-ph[0,-1]**2)*psi_t[0,-1]      
