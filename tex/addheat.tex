...
\begin{equation}
U(c_{\infty},\phi_0) = \frac{1}{1-k} \left( \frac{  c_{\infty}/  c_l^0  }{ (1-\phi_0)/2 + k(1+\phi_0) / 2} - 1\right) 
\end{equation}

\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{./figures/phi_initial.png}
...
symbol & meaning & values & units \\
\midrule
$c_{\infty}|m|$ & shift in melting temperature &  1.2 & T \\
$k$ & interface solute partition coefficient & 0.3 &\\
$T_0$ & reference solidus temperature &  &\\
$T_m$ & melting temperature of pure material $A$ &  &\\
$l_T$ & thermal length &  & L \\
$\gamma$ &  average surface tension &  & \\
$\delta$ & strength of the surface tension anisotropy  &  0.011 &\\
$\Gamma$ & Gibbs-Thompson coefficient & $6.4\times 10^{-8}$ & Km \\
$d_0$ & capillary length & $ 3.75\times10^{-4}$  & L \\
$G$ & thermal gradient & 0.003 & $\text{T} / \text{L}$ \\
$R$ & pulling speed &  20 & $ \text{L} / \text{t}$ \\
$D_l$ & solute diffusion coefficient &$10^{3}$ &  $\text{L}^2/\text{t}$ \\
$L$ & latent heat &  & \\
$W_0$ & interface thickness  & 40  & $d_0$ \\
$\Delta x$ & mesh size & 1.3 & $W_0$ \\
\bottomrule
\end{tabular}
\end{table}
T=1K, L=1$\mu $m, t=1s, non-dimensional parameters
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{./figures/full.png}
\caption{Timeshots for $\phi$}
\end{figure}
